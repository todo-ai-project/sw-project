...
import { getMyCharacter, purchaseCharacterItem } from '../../services/api';
import { useCoins } from '../../context/CoinContext';

const JELLY_COLORS = [
  { name: '하늘', preview: '#BAE6FD' }, { name: '핑크', preview: '#F9A8D4' },
  { name: '민트', preview: '#99F6E4' }, { name: '선샤인', preview: '#FDE68A' },
  { name: '보라', preview: '#C4B5FD' }, { name: '그레이', preview: '#D1D5DB' },
];
...
function ProfilePage() {
  ...
  const [owned, setOwned] = useState(() => readJson('todoongsilOwnedItems', []));
  const [outfit, setOutfit] = useState(() => readJson('todoongsilOutfit', { hat: '', accessory: '', color: '' }));
  const userEmail = localStorage.getItem('userEmail') || '';

  useEffect(() => {
    getMyCharacter().then(character => {
      if (!character) return;
      setOutfit({ hat: character.hat || '', accessory: character.accessory || '', color: character.color || '' });
    }).catch(() => {});
  }, []);

  useEffect(() => localStorage.setItem('todoongsilOwnedItems', JSON.stringify(owned)), [owned]);
  useEffect(() => localStorage.setItem('todoongsilOutfit', JSON.stringify(outfit)), [outfit]);

  const stats = useMemo(() => [
    { label: '보유 코인', value: `${coins}개`, icon: <Coins size={18} />, color: '#B7791F', bg: '#FFF7D6' },
  ...
  const saveName = () => {
    ...
    if (next) { setUserName(next); localStorage.setItem('userName', next); }
    setEditingName(false);
  };

  const equip = (item) => setOutfit(prev => ({ ...prev, [item.type]: prev[item.type] === item.value ? '' : item.value }));

  const buy = async (item) => {
    if (owned.includes(item.id)) { equip(item); return; }
    if (coins < item.price) { alert('코인이 부족해요!'); return; }

    try {
      const data = await purchaseCharacterItem(item.id);
      if (Number.isFinite(Number(data?.coins))) setBalance(Number(data.coins));
      else await refreshCoins();
    } catch (error) {
      console.warn('상점 서버 연동 실패, 프론트 임시 구매로 처리합니다.', error.message);
      const result = spendCoins(item.price);
      if (!result.ok) { alert('코인이 부족해요!'); return; }
    }

    setOwned(prev => [...prev, item.id]);
    setOutfit(prev => ({ ...prev, [item.type]: item.value }));
      ...
      {photoOpen && (
        <div onClick={() => setPhotoOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 60, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, background: 'rgba(12,74,110,.28)', backdropFilter: 'blur(6px)' }}>
          <div onClick={event => event.stopPropagation()} style={{ width: '100%', maxWidth: 390, background: '#fff', borderRadius: 24, padding: 24 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}><h3 style={{ margin: 0, color: C.deep }}>내 해파리 사진 📸</h3><button onClick={() => setPhotoOpen(false)} style={{ border: 0, background: 'none', cursor: 'pointer', color: C.muted }}><X size={18} /></button></div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 3fr', gap: 20 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <Card><div style={{ padding: 24, textAlign: 'center', borderRadius: 24 }}>
              ...
              <button onClick={() => navigate('/shop')} style={{ width: '100%', marginBottom: 8, padding: 10, border: `2px solid ${C.border}`, borderRadius: 16, background: '#F0FBFF', color: C.ocean, fontWeight: 800, cursor: 'pointer' }}>🛍️ 상점 가기</button><button onClick={() => setPhotoOpen(true)} style={{ width: '100%', padding: 10, border: 0, borderRadius: 16, background: GRAD, color: '#fff', fontWeight: 800, cursor: 'pointer', display: 'flex', justifyContent: 'center', gap: 7 }}><Camera size={14} />사진 찍기</button>
            </div></Card>

            <Card><div style={{ padding: 16 }}><p style={{ fontWeight: 800, color: C.deep, marginBottom: 10 }}>해파리 색상</p><div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>{JELLY_COLORS.map((color, index) => <button key={color.name} onClick={() => { setSelColor(index); localStorage.setItem('jellyColor', String(index)); }} style={{ padding: 8, borderRadius: 16, cursor: 'pointer', background: selColor === index ? '#E0F7FF' : '#F8FAFC', border: selColor === index ? '3px solid #0EA5E9' : '3px solid transparent' }}><img src={JELLY_IMAGES[index]} alt={color.name} style={{ width: 46 }} /><span style={{ display: 'block', fontSize: 11, fontWeight: 700, color: C.muted }}>{color.name}</span></button>)}</div></div></Card>
          </div>
          ...
          <div>
            ...
            {tab === 'stat' && <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>{stats.map(stat => <div key={stat.label} style={{ borderRadius: 24, padding: 16, background: stat.bg }}><div style={{ width: 36, height: 36, borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,.65)', color: stat.color }}>{stat.icon}</div><p style={{ fontSize: 23, fontWeight: 800, color: C.deep, marginTop: 8 }}>{stat.value}</p><p style={{ fontSize: 12, color: C.muted }}>{stat.label}</p></div>)}</div>}
