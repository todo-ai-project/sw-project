...
└── src/
    ...
    │   ├── goalController.js         # goals CRUD + AI 분석 트리거
    │   ├── todoController.js         # todos CRUD + 완료 체크 (만다라트 세부목표 포함)
    │   ├── crewController.js         # crews 생성/가입/탈퇴 (소셜 방)
    │   └── characterController.js    # characters 꾸미기
    ├── routes/
    │   ├── authRoutes.js
    │   ├── goalRoutes.js
    │   ├── todoRoutes.js
...
| 컬렉션 | 필드 | 설명 |
|---|---|---|
| `users/{uid}` | createdAt, crewId, email, nickname | 문서 ID = Firebase Auth uid |
